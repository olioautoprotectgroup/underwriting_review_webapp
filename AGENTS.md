...
This file applies to any AI coding agent working in this repo — Codex,
Claude Code, or otherwise. (Claude Code also reads `CLAUDE.md`, which
carries the same rules below plus a couple of Claude-specific notes; read
whichever your tooling looks for, or both.)
...
avoid repeating it: multiple agents and the repo owner all have push
access, so a direct push collides silently.
